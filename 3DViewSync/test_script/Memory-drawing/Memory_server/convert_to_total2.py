...
def get_format_list(input_file):
	...
	for line in input_file:
		if(line != '\n'): # delete blank line
			line = line.split()
			# print int(line[1].rstrip('M'))
			format_list.append(int(line[1].replace('M','')))
			if(line[0] == "PhysMem:"): continue
	return format_list
...
for input_filename in list_input_filename:
	...
	client_of_number = int(input_filename.replace('server','').replace('.txt',''))
